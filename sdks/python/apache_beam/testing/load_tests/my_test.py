...
class ParDoTest(unittest.TestCase):
  ...
  def testParDo(self):
    ...
    with self.pipeline as pipeline:
      pc = (pipeline
            | 'Read IO' >> beam.io.Read()
            | beam.WindowInto(
              window.FixedWindows(),
              trigger=trigger.AfterWatermark(trigger.AfterWatermark()),
              accumulation_mode=AccumulationMode.ACCUMULATING
          )
            | 'Get values' >> beam.ParDo(ReadValues())
            | 'Mean' >> beam.CombineValues(beam.combiners.MeanCombineFn())
            | 'Write' >> beam.io.Write()
            )
